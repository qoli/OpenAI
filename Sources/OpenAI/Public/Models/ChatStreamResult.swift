...
public struct ChatStreamResult: Codable, Equatable {
    ...
    public struct Choice: Codable, Equatable {
        ...
        public struct ChoiceDelta: Codable, Equatable {
            ...
            /// Creates a ChoiceDelta from a dictionary,提供更安全的初始化方式
        ...
        public init(from decoder: Decoder) throws {
            ...
            // 安全地解析 delta
            if let deltaContainer = try? container.nestedContainer(keyedBy: ChoiceDelta.CodingKeys.self, forKey: .delta) {
               let role = try deltaContainer.decodeIfPresent(ChoiceDelta.Role.self, forKey: .role)
               let content = try deltaContainer.decodeIfPresent(String.self, forKey: .content)
               let reasoningContent = try deltaContainer.decodeIfPresent(String.self, forKey: .reasoning_content)
               let toolCalls = try deltaContainer.decodeIfPresent([ChoiceDelta.ChoiceDeltaToolCall].self, forKey: .toolCalls)
               
               self.delta = ChoiceDelta(
                   content: content,
